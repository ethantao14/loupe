...
def test_rejects_malformed_url(memory_store):
    assert "Not a valid URL" in tools.run_tool("fetch_url", {"url": "http://[bad"}, memory_store)
...
def test_rejects_unknown_tool(memory_store):
    assert "Unknown tool" in tools.run_tool("rm_rf", {}, memory_store)
...
def test_fetch_returns_text(monkeypatch, memory_store):
    ...
    assert tools.run_tool("fetch_url", {"url": "https://example.com"}, memory_store) == "Loupe docs"
...
def test_remember_dispatches(memory_store):
    ...
    assert tools.run_tool("remember", {"fact": fact}, memory_store) == f"Remembered: {fact}"
...
def test_remember_rejects_missing_or_non_string_fact(tool_input, memory_store):
    assert tools.run_tool("remember", tool_input, memory_store) == "Error: fact must be a string."
    memory_store.remember.assert_not_called()


@pytest.mark.parametrize("fact", ["", " \n\t"])
def test_remember_rejects_blank_fact(fact, memory_store):
    assert tools.run_tool("remember", {"fact": fact}, memory_store) == (
...
def test_remember_returns_helpful_storage_error(memory_store):
    ...
    assert tools.run_tool("remember", {"fact": "A fact"}, memory_store) == (
        "Error: Could not save memory. Please try again."
    )
